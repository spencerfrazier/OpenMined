...
namespace OpenMined.Syft.Tensor
{
    public partial class IntTensor
    {
	    ...
	    public IntTensor HookGraph(ref IntTensor result, 
		    						string creation_op, 
		    						bool inline, 
		    						int scalar_input = -1, 		    
		    						IntTensor[] tensor_inputs = null, 
	    ...
	    {		    
		    ...
		    if (result == null)
		    {
				...
				for (int i = 0; i < this.children_indices.Count; i++)
				{
					...
					IntTensor child = factory.Get(children_indices[i]);
					...
					if (child.creation_op == creation_op)
					{
						...
						if (tensor_inputs != null && tensor_inputs.Length >= 1)
							foreach(IntTensor tensor in tensor_inputs)
